...
template<typename T>
class Run2JetConstituentTableProducer : public edm::stream::EDProducer<> {
public:
  explicit Run2JetConstituentTableProducer(const edm::ParameterSet &);
  ~Run2JetConstituentTableProducer() override;
...
//
template< typename T>
Run2JetConstituentTableProducer<T>::Run2JetConstituentTableProducer(const edm::ParameterSet &iConfig)
...
template< typename T>
Run2JetConstituentTableProducer<T>::~Run2JetConstituentTableProducer() {}

template< typename T>
void Run2JetConstituentTableProducer<T>::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  // PF Cands
  std::vector<float> btagEtaRel, btagPtRatio, btagPParRatio, btagSip3dVal, btagSip3dSig, btagSip2dVal, btagSip2dSig, btagJetDistVal;
  // Secondary vertices
    ...
    for (const auto &sv : jetSVs) {
      ...
      if (readBtag_ && !vtxs_->empty()) {
        ...
        sv_chi2.push_back(sv->vertexChi2());
       	sv_normchi2.push_back(btagbtvdeep::catch_infs_and_bound(sv->vertexChi2() / sv->vertexNdof(), 1000, -1000, 1000));
        const auto& dxy_meas = btagbtvdeep::vertexDxy(*sv, *pv_);
        sv_dxy.push_back(dxy_meas.value());
	sv_dxysig.push_back(btagbtvdeep::catch_infs_and_bound(dxy_meas.value() / dxy_meas.error(), 0, -1, 800));
        const auto& d3d_meas = btagbtvdeep::vertexD3d(*sv, *pv_);
        sv_d3d.push_back(d3d_meas.value());
	sv_d3dsig.push_back(btagbtvdeep::catch_infs_and_bound(d3d_meas.value() / d3d_meas.error(), 0, -1, 800));
        sv_costhetasvpv.push_back(btagbtvdeep::vertexDdotP(*sv, *pv_));
        // Jet related
        sv_ptrel.push_back(sv->pt() / jet.pt());
        sv_phirel.push_back(reco::deltaPhi(*sv, jet));
	sv_deltaR.push_back(btagbtvdeep::catch_infs_and_bound(std::fabs(reco::deltaR(*sv, jet_dir)) - 0.5, 0, -2, 0));
...
template< typename T>
void Run2JetConstituentTableProducer<T>::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
...
typedef Run2JetConstituentTableProducer<pat::Jet> Run2PatJetConstituentTableProducer;
typedef Run2JetConstituentTableProducer<reco::GenJet> Run2GenJetConstituentTableProducer;

DEFINE_FWK_MODULE(Run2PatJetConstituentTableProducer);
DEFINE_FWK_MODULE(Run2GenJetConstituentTableProducer);
